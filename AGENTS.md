...
A general operating manual for any web design project. Project-specific details (stack, brand, file structure) belong in a separate AGENTS.md layered on top of this one.
...
- **Chrome DevTools MCP** — the primary tool for screenshotting, inspecting, navigating, and debugging rendered pages. Use it for both passes of the screenshot workflow. If it's connected, always prefer it over CLI alternatives. If it is not connected, please render the website locally, take the screenshots, and debug as necessary.
- **`frontend-design` skill** at located in the root of this project folder — read this before building any web UI. It encodes design tokens, component patterns, and styling constraints specific to this environment. Loading it is not optional for design work.
...
If Chrome DevTools MCP isn't connected, say so explicitly rather than skipping the verification step. Provide the best, free alternatives that best suite the website creation guidelines.
...
Backend, database, auth, API, DevOps. Project-specific stack choices, brand tokens, file layout. Decision logs. Those belong in a project-level AGENTS.md layered on top of this one — keep that file short too.